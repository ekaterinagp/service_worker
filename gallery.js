...
fetch("https://valsdottir.net/kea/07-cms/wordpress/wp-json/wp/v2/categories")
  .then(e => e.json())
  .then(buildMenu);
...
function fetchGallery() {
  ...
  let endpoint =
    "https://valsdottir.net/kea/07-cms/wordpress/wp-json/wp/v2/artist?_embed&per_page=100";
  if (catid) {
...
function showSinglePiece(aPiece) {
  ...
  if (aPiece._embedded["wp:featuredmedia"][0].media_details.sizes.large) {
    clone
      ...
      .setAttribute(
        "src",
        aPiece._embedded["wp:featuredmedia"][0].media_details.sizes.large
          .source_url
      );
  } else {
    clone
