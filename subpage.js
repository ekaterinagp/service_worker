...
fetch(
  "https://valsdottir.net/kea/07-cms/wordpress/wp-json/wp/v2/artist/" +
    id +
    "?_embed"
)
  .then(e => e.json())
...
function showSingleArt(aArt) {
  ...
  document.querySelector(".price-more span").textContent =
  ...
  if (aArt._embedded["wp:featuredmedia"][0].media_details.sizes.large) {
    //img is there
    document
      .querySelector(".img-more")
      .setAttribute(
        "src",
        aArt._embedded["wp:featuredmedia"][0].media_details.sizes.large
          .source_url
      );
  } else {
    document
      ...
      .setAttribute(
        "src",
        aArt._embedded["wp:featuredmedia"][0].media_details.sizes.medium
          .source_url
      );
  }
  ...
  cartItem.id = aArt.id;
  cartItem.img =
    aArt._embedded["wp:featuredmedia"][0].media_details.sizes.medium.source_url;
...
function fetchSimilarItems(categoryId) {
  fetch(
    "https://valsdottir.net/kea/07-cms/wordpress/wp-json/wp/v2/artist?_embed&per_page=6" +
      "&categories=" +
      categoryId
  )
    .then(e => e.json())
